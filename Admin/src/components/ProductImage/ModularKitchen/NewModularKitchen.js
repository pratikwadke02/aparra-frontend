...
const ProductImageTabs = () => {
  ...
  useEffect(() => {
    const getModularKitchenTableData = async () => {
      try{
        const {data} = await axios.get('http://localhost:5000/api/aparra/getModularKitchens');
  ...
  const handleSubmit = async(e) => {
    ...
    try{
      ...
      await axios.post(`http://localhost:5000/api/aparra/addModularKitchen`, formData).then((res) => {
        console.log(res);
      }
      ).catch((err) => {
        console.log(err);
